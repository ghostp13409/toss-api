...
impl GolangParser {
    fn parse_go_type(type_str: &str) -> FieldType {
        ...
        match type_str.to_lowercase().as_str() {
            ...
            t if t.starts_with("[]") => FieldType::Array(Box::new(Self::parse_go_type(&t[2..]))),
...
impl SourceParser for GolangParser {
    fn parse(&self, project_path: &Path) -> anyhow::Result<Collection> {
        ...
        let field_regex = Regex::new(
            r#"^\s+([a-zA-Z0-9_]+)\s+([a-zA-Z0-9_\[\]\*]+)(?:\s+`json:"([a-zA-Z0-9_]+)".*`)?"#,
        )
        .unwrap();
                    ...
                    if let Some(cap) = struct_regex.captures(lines[i]) {
                        ...
                        while i < lines.len() && !lines[i].contains('}') {
                            if let Some(fcap) = field_regex.captures(lines[i]) {
                                let field_name = fcap
                                    .get(3)
                                    .map_or(fcap[1].to_string(), |m| m.as_str().to_string());
                                fields.push(ModelField {
                                    name: field_name,
                                    field_type: Self::parse_go_type(
                                        &fcap[2].trim_start_matches('*'),
                                    ),
                                });
                            }
                            i += 1;
        ...
        let route_regex =
            Regex::new(r#"\.(?:HandleFunc|GET|POST|PUT|PATCH|DELETE)\s*\(\s*['"]([^'"]+)['"]"#)
                .unwrap();
            ...
            .filter(|e| e.path().extension().map_or(false, |ext| ext == "go"))
        {
            if let Ok(content) = std::fs::read_to_string(entry.path()) {
                ...
                for cap in route_regex.captures_iter(&content) {
                    let url_path = &cap[1];
                    let method_match = Regex::new(r#"\.(GET|POST|PUT|PATCH|DELETE|HandleFunc)"#)
                        .unwrap()
